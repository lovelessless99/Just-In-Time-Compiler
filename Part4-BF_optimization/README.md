...
因此，我們在這裡只要把前面的 compiler 輸出組語的部分直接轉成 opcode 就好。如果不知道怎麼轉，請參考**Part 1. 揉合 objdump**
那段。
...
如果你可能對組合語言有些熟悉的話，又覺得前面直接放入 opcode 到程式碼內可讀性很低，那可以用 dynasm code generator 幫助我們組語寫完轉化成 opcode，程式碼放進 `opt_2.c` 檔案內
...
```
比方才的程式碼稍微簡潔不複雜，可讀性也提高了(不過組語本來就是提高機器語言的可讀性建立出來的高階語言(跟機器語言比的話)🤣)

### 4.3-2. Contraction



其實改動的地方很簡單，最主要是 `>`, `<`, `+`, `-` 的地方，如下
```C
for (char *p = argv[1]; *p; p++) {
    switch (*p) {
      case '>':
...
如此一來，可以先利用 `continuous_count` 先計算未來的連續符號個數，例如未來有五個 +，就可以把五個加法指令合併成一個加法指令`add PTR, 5`
