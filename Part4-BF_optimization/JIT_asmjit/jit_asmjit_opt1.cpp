...
int main(int argc, char *argv[]) {
  ...
  // target architecture, operating system, ABI, and other important
  // properties.
  code.init(rt.environment(), rt.cpuFeatures());

  // Emitters can emit code to CodeHolder - let's create 'x86::Assembler',
  // which can emit either 32-bit (x86) or 64-bit (x86_64) code. The following
  // line also attaches the assembler to CodeHolder, which calls
  // 'code.attach(&a)' implicitly.
  ...
  for (char *p = file_contents; *p; p++) {
    ...
    case '+':
      count = continuous_count(p);
      p += count - 1;
      as.add(x86::byte_ptr(PTR), count);
      break;
    case '-':
      count = continuous_count(p);
      p += count - 1;
      as.sub(x86::byte_ptr(PTR), count);
      break;
    case '.':
